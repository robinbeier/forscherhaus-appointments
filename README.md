...
CI note: the `coverage-delta` check validates Clover line coverage against the in-repo baseline/delta policy and is blocking. The coverage suite includes Unit tests plus booking flow integration tests. On pull requests it runs only when the `ci/full` label is present.
