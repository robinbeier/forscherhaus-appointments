...
use AppointmentApiOverlapException;
...
final class AppointmentsApiUpdateStatusTest extends TestCase
{
    ...
    public function testExpectedUpdateRaceStatusesUseGenericJsonResponse(): void
    {
        ...
        foreach (
            [
                new AppointmentApiUpdateException('internal concurrency detail', 404),
                new AppointmentApiUpdateException('internal concurrency detail', 409),
                new AppointmentApiOverlapException('internal overlap detail', 409),
            ]
            as $exception
        ) {
            get_instance()->output->set_output('');

            self::assertTrue($method->invoke($controller, $exception));
            self::assertSame($exception->getCode(), get_instance()->output->statusCode);
            ...
            self::assertStringNotContainsString('internal overlap detail', get_instance()->output->get_output());
        }
