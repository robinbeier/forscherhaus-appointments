...
final class DeploymentHostRunnerDumpV1Test extends TestCase
{
    ...
    public function testCollectorPinsExactDumpBeforeReadingDigestDerivedAttestation(): void
    {
        $attestation = DeploymentEvidenceAuthorityV1::encodeFile([
            ...
            'dump' => [
                'sha256' => self::SHA, 'size_bytes' => 1_000_000,
                'uncompressed_size_bytes' => 4_000_000, 'created_at_utc' => '2026-08-12T11:30:00Z',
            ],
            'verification' => [
                ...
                'restored_datadir_inode_count' => 256, 'restored_at_utc' => '2026-08-12T11:50:00Z',
            ],
        ...
        $helper = new DumpHelperFake([
            ...
            'attestation_sha256' => hash('sha256', $attestation), 'dump_sha256' => self::SHA,
            'dump_size_bytes' => 1_000_000, 'observed_at_utc' => '2026-08-12T12:00:00Z',
        ], $storage);

        $result = (new ProtectedHostDumpCollector($storage, $helper))->collect(
            self::RUN_ID,
            ['path' => '/root/backups/predeploy.sql.gz', 'sha256' => self::SHA],
        );
    ...
    public function testMissingAttestationRetainsOnlyIndependentlyVerifiedDumpDigest(): void
    {
        ...
        $helper = new DumpHelperFake([
            ...
            'dump_sha256' => self::SHA, 'dump_size_bytes' => 1_000_000,
            'observed_at_utc' => '2026-08-12T12:00:00Z',
        ], $storage);

        $result = (new ProtectedHostDumpCollector($storage, $helper))->collect(
            self::RUN_ID,
            ['path' => '/root/backups/predeploy.sql', 'sha256' => self::SHA],
        );
    ...
    public function testCallerCannotSelectAnotherDumpLeafOrMalformedDigest(): void
    {
        ...
        (new ProtectedHostDumpCollector(new DumpStorageFake(), new DumpHelperFake(null, new DumpStorageFake())))
