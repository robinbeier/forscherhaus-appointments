...
test('sanitizes policy violations into fixed classes without raw locations', () => {
    ...
    assert.equal(probe.blockedOriginClass('http://[::1]:8080/private'), 'self');
});
...
test('receipt contains only aggregate classes and explicit non-production markers', () => {
    ...
    const receipt = probe.makeReceipt(
        'booking',
        [probe.sanitizeViolation({effectiveDirective: 'script-src', blockedURI: raw}, 'booking')],
        true,
    );
...
test('classifies loopback, browser-local, and external requests without retaining URLs', () => {
    assert.equal(probe.requestClass('http://127.0.0.1:8080/app.js'), 'loopback');
    assert.equal(probe.requestClass('http://[::1]:8080/app.js'), 'loopback');
...
test('unknown surfaces and directives fail closed to safe classes', () => {
    const violation = probe.sanitizeViolation(
...
const record = {header: null, closed: false, launched: false, externalFetched: false, externalAborted: false};
...
`,
    ...
    try {
        ...
        assert.equal(receipt.violation_count, 1);
        assert.equal(receipt.blocked_request_count, 1);
        assert.deepEqual(receipt.blocked_request_classes, {external: 1});
