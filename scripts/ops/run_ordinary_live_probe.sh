...
case "$action" in
    preflight|account|methods|customer-boundary|customers-api|services-api|calendar-race|appointments-api|appointments-api-overlap|session|cleanup|verify) ;;
...
[[ "$action" != customer-boundary && "$action" != customers-api && "$action" != services-api ]] || fixture_role=admin
