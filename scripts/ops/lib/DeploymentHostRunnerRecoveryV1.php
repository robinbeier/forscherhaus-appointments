...
final class HostRunnerRecoveryAdmission
{
    ...
    public function __construct(
        ...
        $this->start =
            $start ??
            new HostRunnerStartOrchestrator(
    ...
    public function admit(array $request, array $input): array
    {
        ...
        if ($state['state'] === 'rollback_running') {
            ...
            $resumed = DeploymentHostRunnerContractV1::decodeState($this->required($prefix . 'state.json', 4_096));
            if (in_array($resumed['state'], DeploymentContractV1::TERMINAL_FAILURE_STATES, true)) {
                return $this->terminal->resumeTerminal($runId, 'recovery');
            }
            if (
        ...
        $claim = [
            ...
            'events_sha256' => hash('sha256', $candidateEvents),
    ...
    {
        return $this->storage->read($relative, $limit) ??
            throw new RuntimeException('recovery admission authority is incomplete');
    }
