...
try {
    ...
    if ($action === 'preflight') {
        ...
        if ($action === 'account') {
            ...
            $probe = CustomersApiWriteProbe::forApp(
                'http://localhost',
            ...
            $result['evidence'] = $probe->run($evidence->step(...));
        } elseif ($action === 'unavailabilities-api') {
            $evidence->run(
                'supplemental_activate',
                fn(): array => $verificationFixture->activate('unavailabilities_api', $context),
            );
            $probe = UnavailabilitiesApiWriteProbe::forApp(
                'http://localhost',
                (string) $context['username'],
                (string) $context['password'],
                $verificationFixture,
            ...
            $result['evidence'] = $probe->run($evidence->step(...));
        } elseif ($action === 'service-categories-api') {
            $evidence->run(
                'supplemental_activate',
                fn(): array => $verificationFixture->activate('service_categories_api', $context),
            );
            $probe = ServiceCategoriesApiWriteProbe::forApp(
                'http://localhost',
                (string) $context['username'],
                (string) $context['password'],
                $verificationFixture,
            ...
            $result['evidence'] = $probe->run($evidence->step(...));
        } elseif ($action === 'secretaries-api') {
            $evidence->run(
                'supplemental_activate',
                fn(): array => $verificationFixture->activate('secretaries_api', $context),
            );
            $probe = SecretariesApiAliasProbe::forApp(
                'http://localhost',
                (string) $context['username'],
                (string) $context['password'],
                $verificationFixture,
