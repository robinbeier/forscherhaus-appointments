...
- For daily application-log inspection and occasional approved manual cleanup,
  see [`docs/ops/production-app-log-retention.md`](../../docs/ops/production-app-log-retention.md).
...
- `prod_csp_report_only_pilot.sh` runs one read-only preflight or, after separate
  ...
  repeated automatically. The journal is removed only after successful
  postflight verification. It retains each completed checkpoint's classified
  result and the activation completion time, so resumed 15/60-minute waits use
...
- `run_read_only_http_probe.sh` performs four bounded anonymous GET checks through
  ...
  routes and switches to a non-persistent session driver; the existing loopback
  rate-limit guard and probe-only ICS 404 logging suppression keep the run
