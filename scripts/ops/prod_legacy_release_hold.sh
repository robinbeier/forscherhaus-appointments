...
result_file="$(mktemp /tmp/fh-legacy-release-hold.XXXXXX)"
trap 'rm -f -- "$result_file"' EXIT
set +e
ssh "${SSH_OPTIONS[@]}" "$PROD_SSH_TARGET" "$remote_command" >"$result_file" 2>/dev/null
ssh_status=$?
set -e
result_size="$(wc -c <"$result_file")"
result_size="${result_size//[[:space:]]/}"
result_invalid=0
if [[ ! "$result_size" =~ ^[0-9]+$ ]] || (( result_size > 4096 )); then
  result_invalid=1
  result=''
else
  result="$(<"$result_file")"
...
if ! validated_result="$(HOLD_RESULT="$result" HOLD_MODE="$([[ $PROVISION == 1 ]] && echo provision || echo inspect)" HOLD_SSH_STATUS="$ssh_status" /usr/bin/python3 -c 'import json,os,sys
try:
    value=json.loads(os.environ["HOLD_RESULT"])
    mode=os.environ["HOLD_MODE"]
    ssh_status=int(os.environ["HOLD_SSH_STATUS"])
