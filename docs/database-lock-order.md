...
| Calendar manual unavailability save/delete | Outer transaction; locks the current and requested provider users in numeric order, then the manual unavailability row. For an update or delete, the locked provider must still match the earlier authorized record, and current event permissions are checked before mutation. Model writes join the outer transaction. This does not establish a lock protocol for unrelated role or secretary-assignment writes. |
...
| Secretary save and provider-assignment replacement | Own or join the outer transaction. Lock the existing secretary and requested provider users as one sorted, deduplicated set before settings or assignment changes; validate the roles from those locked rows. New secretary insertion follows provider-parent locking. Repeated assignment validation inside save reacquires only already-held parents. This does not establish an ordering guarantee for unrelated staff maintenance/deletion paths. |
