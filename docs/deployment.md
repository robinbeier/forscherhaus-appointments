...
- direct regular `ea_session*` files with one link under `storage/sessions`
  are reset to mode `0600` after each generic stage permission pass; symlinks
  and their targets are not followed
