...
Playwright runtime version declared by the pinned `@playwright/cli` package.
Advanced local debugging can still override the runtime package explicitly via
`PLAYWRIGHT_RUNTIME_PACKAGE`. The wrapper then prepares the required Linux
browser dependencies inside the validation container. Local and CI gate runs
therefore use the same browser/runtime contract across Linux architectures.
