...
| Primary-only GitHub PR metadata writes | `docs/github-pr-write-transport.md`, `scripts/agent/github_pr_write_transport.php` | Bounded stdin-only PR title/body and exact new-comment REST writes, bound to the canonical repository and local exact head through an absolute verified native `gh` binary. |
