...
| Primary-only GitHub PR metadata writes | `docs/github-pr-write-transport.md`, `scripts/agent/github_pr_write_transport.php` | Bounded stdin-only PR title/body and exact new-comment REST writes through an absolute verified native `gh` binary, with canonical local exact-head-and-branch preflight/postflight, non-retryable completed-write uncertainty, and no landing authority. |
