...
| External read-only final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh` | Review-base-anchored trust paths derived and validated from one structured contract, plus schema, ambient-PHP/Git-config-free validation, fixed tool resolution, private exact-commit checkout, disabled web search, read-only filesystem, denied command network, no external connectors, fail-closed exact-head JSON output, and an explicit external bootstrap boundary. |
