...
| Controlled parallel implementation | `WORKFLOW.md`, `scripts/agent/check_parallel_work_contract.sh` | Wrapper materialization through absolute system Git, live unauthenticated read-only binding to the fixed public canonical main plus an exactly matching local tracking ref, `php -n` entry with an explicit Bash environment allowlist, declared-base PHP orchestration/policy/ownership/path-validator blobs, PHP-startup-isolated admission, trusted pre-commit verification of actual lane paths, and clean post-commit binding to the immutable lane head; canonical PHP lane-path grammar plus one shared Python ownership-map rule parser, disjoint explicit ownership, semantic-independence attestation, canonical component approval, primary-reserved paths, and a two-lane maximum with serial external writes and landing. |
| External sealed-bundle final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh` | Live unauthenticated read-only binding to the fixed public canonical main plus an exactly matching tracking ref, hardened system-Git base bootstrap, one exact policy resolver for profile/schema/features/trust paths, exact Codex 0.145.0 platform/release digest plus private-copy pin, deterministic SHA-256-bound patch/base/head serialization over stdin, outer macOS Seatbelt default-deny with exact bundle and read-only host-login access, isolated runtime state, tool-free single-model catalog, temp/home/original-worktree denial canaries, no connectors or delegation, and fail-closed exact-head/exact-diff JSON output; other platforms stop closed. |
