...
| External read-only final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh` | Review-base-anchored trust paths derived and validated from one structured contract, plus schema, clean shell bootstrap with fixed system temporary storage, canonical physical root and executable targets, ambient-PHP/Git-config-free validation, disabled lazy object fetching, fixed and identity-checked tool resolution, private exact-commit checkout, disabled web search, read-only filesystem, denied command network, no external connectors, fail-closed exact-head and exact-diff-path-bound JSON output, and an explicit external bootstrap boundary. |
