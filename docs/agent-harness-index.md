...
| Controlled parallel implementation | `WORKFLOW.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/check_parallel_work_contract.sh` | Exact-base, live-main-bound admission and verification for at most two disjoint local worker lanes; primary-only external mutation and serial integration/landing. |
| External sealed-bundle final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh`, `scripts/agent/lib/readonly_reviewer_*_runtime.sh` | Exact-base/exact-head deterministic bundle, macOS Seatbelt default-deny isolation, pinned no-tool reviewer runtime, fail-closed privacy-safe output; no worktree, credentials, connectors, delegation, or external writes. |
