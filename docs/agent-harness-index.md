...
| External read-only final reviewers | `code_review.md`, `scripts/agent/run_readonly_reviewer.sh` | Profile-derived ephemeral sessions with read-only filesystem, denied command network, no external connectors, and fail-closed exact-head JSON output. |
