...
| Controlled parallel implementation | `WORKFLOW.md`, `scripts/agent/check_parallel_work_contract.sh` | PHP-startup-isolated, base-commit-anchored policy and ownership map, one shared normalized repository-path grammar, disjoint ownership with explicit per-rule match types, semantic-independence attestation, canonical component approval, primary-reserved paths, and a two-lane maximum with serial external writes and landing. |
| External read-only final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh` | Review-base-anchored trust paths derived and validated from one structured contract, plus schema, clean shell bootstrap, ambient-PHP/Git-config-free validation, disabled lazy object fetching, fixed and identity-checked tool resolution, private exact-commit checkout, disabled web search, read-only filesystem, denied command network, no external connectors, fail-closed exact-head JSON output, and an explicit external bootstrap boundary. |
