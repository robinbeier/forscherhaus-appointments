...
| Controlled parallel implementation | `WORKFLOW.md`, `scripts/agent/check_parallel_work_contract.sh` | Materialized declared-base wrapper and PHP blobs, PHP-startup-isolated admission, trusted pre-commit verification of actual lane paths, and clean post-commit binding to the immutable lane head; one shared control-character-safe repository-path grammar, disjoint explicit ownership, semantic-independence attestation, canonical component approval, primary-reserved paths, and a two-lane maximum with serial external writes and landing. |
| External read-only final reviewers | `code_review.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/run_readonly_reviewer.sh` | Review-base-anchored trust paths derived and validated from one structured contract, plus schema, clean shell bootstrap with fixed system temporary storage, canonical physical root and executable targets, ambient-PHP/Git-config-free validation, disabled lazy object fetching, fixed and identity-checked tool resolution, private symlink-free exact-commit checkout, disabled web search, read-only filesystem, denied command network, no external connectors, fail-closed exact-head and exact-diff-path-bound JSON output, and an explicit external bootstrap boundary. |
