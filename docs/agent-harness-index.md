...
| Primary-only GitHub PR metadata writes | `docs/github-pr-write-transport.md`, `.codex/contracts/agent-workflow.json`, `scripts/agent/github_pr_write_transport.php`, `scripts/agent/lib/GithubPrWrite*.php` | Bounded stdin-only PR title/body and exact new-comment REST writes through a source-handle-bound, digest-attested private copy of the native `gh` binary loaded from the canonical machine manifest, with split request/runtime/process/target modules, concurrent bounded nonblocking pipe I/O, fixed minimal remote-response projections, canonical local exact-head-and-branch preflight/postflight plus created-comment ID/target/body readback, non-retryable completed-write uncertainty, and no landing authority. |
