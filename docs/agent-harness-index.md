...
| Controlled parallel implementation | `WORKFLOW.md`, `scripts/agent/check_parallel_work_contract.php` | Common-base, disjoint-ownership, canonical component approval, primary-reserved paths, and two-lane maximum with serial external writes and landing. |
| External read-only final reviewers | `code_review.md`, `scripts/agent/run_readonly_reviewer.sh` | Review-base-anchored profiles, schema, validator, and runner in ephemeral sessions with read-only filesystem, denied command network, no external connectors, fail-closed exact-head JSON output, and an explicit external bootstrap boundary. |
