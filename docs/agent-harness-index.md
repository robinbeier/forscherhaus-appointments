...
| Primary-only GitHub PR metadata writes | `docs/github-pr-write-transport.md`, `scripts/agent/github_pr_write_transport.php` | Allowlisted PR title/body and exact new-comment REST writes through native `gh` authentication without token export or Projects scope. |
