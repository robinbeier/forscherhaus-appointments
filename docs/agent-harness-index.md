...
| Primary-only GitHub PR metadata writes | `docs/github-pr-write-transport.md`, `scripts/agent/github_pr_write_transport.php` | Bounded stdin-only PR title/body and exact new-comment REST writes through a source-handle-bound, digest-attested private copy of the absolute verified native `gh` binary, with fixed minimal remote-response projections, canonical local exact-head-and-branch preflight/postflight plus created-comment ID/target/body readback, non-retryable completed-write uncertainty, and no landing authority. |
