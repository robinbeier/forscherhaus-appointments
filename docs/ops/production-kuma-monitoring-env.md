...
`alias`, `unalias`, `declare`, `enable`, `hash`, `let`, `readonly`, `set`,
`shopt`, `trap` and `typeset` commands are unsupported because they can change
later command resolution, variable attributes, shell behavior, arithmetic
status or the appended assignment itself. A `command_not_found_handle`
