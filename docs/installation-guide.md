...
There are 6 steps you must follow during the installation process.
  ...
  1. **Make sure that your server has at least the following applications/tools installed: Apache(v2.4), PHP(v8.1+) and MySQL(v5.7+).** Easy!Appointments needs these programs to run. Most of the web hosting companies provide these tools within their Linux hosting plans. If you want to install Easy!Appointments on your local server use one of the pre-made bundles available on the web (XAMPP, MAMP, WAMP ...), all of which are free to use. PHP should have the `curl`, `json`, `mbstring`, `gd`, `simplexml`, and `fileinfo` extensions enabled (the `curl` extension is required for Google Calendar synchronization).
