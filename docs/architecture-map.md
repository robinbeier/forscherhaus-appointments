...
Every path rule declares `match: directory`, `match: exact_file`, or `match: filename_prefix`; path spelling has no implicit match semantics. Canonical Python ownership matching cases live in `.codex/contracts/ownership-path-rules.json`.
...
|---|---|---|---:|---:|
| `auth-session` | Access & Session | integrations-sync, people-services-admin, scheduling-backoffice, settings-compliance, shared-core | 17 | 3 |
...
- `application/libraries/Session/drivers/Session_null_driver.php` (exact_file)
